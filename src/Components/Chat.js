...
const Chat = () => {
  ...
  useEffect(()=>{
    db.collection('fireChat').orderBy('sentAt').limit(50).onSnapshot(snapshot=>{
      ...
      <div className="container-fluid chatwrapper">
        ...
        <div className="row chatHeader pt-2 pb-2 ps-5 ps-lg-1 pe-1 pe-lg-5 position-fixed top-0 w-100">
          <div className="col-3  d-flex justify-content-start align-items-center">
            ...
            <div className="bottom" aria-hidden="true">FireChat</div>      
            </section>
          </div>
        ...
        <div className="chatCenter">
          {messages.map(({ text, photoURL, uid}, i) => {
            return(
              <div 
              className={`mt-3 ${uid === auth.currentUser.uid ? `sender` : `reciver` }`}
              key={i} >

                <img  
                className='pro'
                src={photoURL} alt="" width={45} height={45} />
